...
export async function createApp(projectRoot, options = {}) {
  ...
  app.post('/api/playgrounds/:name/reset', (req, res) => {
    ...
    try {
      const wtPath = campPath(name);
      spawnSync('git', ['-C', wtPath, 'fetch', 'origin'], { stdio: 'pipe' });
      spawnSync('git', ['-C', wtPath, 'reset', '--hard', `origin/${pg.branch}`], { stdio: 'pipe' });
      spawnSync('git', ['-C', wtPath, 'clean', '-fd'], { stdio: 'pipe' });
      writeActions(name, []);
  ...
  app.get('/api/playgrounds/:name/changes', (req, res) => {
    ...
    try {
      const wtPath = campPath(name);
      const diff = (spawnSync('git', ['-C', wtPath, 'diff', '--name-status'], { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] }).stdout || '').trim();
      const untracked = (spawnSync('git', ['-C', wtPath, 'ls-files', '--others', '--exclude-standard'], { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] }).stdout || '').trim();
      const files = [];
      if (diff) {
  ...
  app.post('/api/playgrounds/:name/ship', async (req, res) => {
    ...
    try {
      ...
      const commitResult = spawnSync('git', ['-C', wtPath, 'commit', '-m', message, '--allow-empty'], { stdio: 'pipe' });
  ...
  app.post('/api/playgrounds/:name/sync', (req, res) => {
    ...
    try {
      ...
      const result = (mergeResult.stdout || '').trim();
      if (result.includes('CONFLICT')) {
        try { spawnSync('git', ['-C', wtPath, 'merge', '--abort'], { stdio: 'pipe' }); } catch {}
      ...
      if (msg.includes('CONFLICT')) {
        try { spawnSync('git', ['-C', campPath(name), 'merge', '--abort'], { stdio: 'pipe' }); } catch {}
        res.json({ synced: false, conflict: true, message: '충돌이 있어서 자동 반영이 안 됩니다.' });
      } else {
        res.status(500).json({ error: err.message });
      }
