...
export async function createApp(projectRoot, options = {}) {
  ...
  app.post('/api/playgrounds', async (req, res) => {
    ...
    try {
      ...
      const record = { name, branch, slot, fePort, bePort, status: 'setting-up' };
      upsert(record);
  ...
  app.post('/api/quick-start', async (req, res) => {
    ...
    try {
      ...
      copyCampFiles(projectRoot, wtPath, config.copyFiles, (msg) => {
        broadcast({ type: 'log', name, source: 'sanjang', data: msg });
      });

      const record = { name, branch, slot, fePort, bePort, status: 'setting-up', description: description.trim() };
      upsert(record);
      broadcast({ type: 'playground-created', name, data: record });
      res.status(201).json(record);
      ...
      if (config.setup) {
        ...
        if (cacheResult.applied) {
          ...
          broadcast({ type: 'log', name, source: 'sanjang', data: `캐시 없음 (${cacheResult.reason}), 설치 중 (${config.setup})...` });
          const setupProc = spawn(config.setup, [], {
            cwd: wtPath, shell: true, stdio: ['ignore', 'pipe', 'pipe'],
          });
          setupProc.stdout.on('data', (d) => {
            broadcast({ type: 'log', name, source: 'setup', data: d.toString() });
