...
export async function createApp(projectRoot, options = {}) {
  ...
  app.post('/api/quick-start', async (req, res) => {
    ...
    try {
      ...
      if (config.setup) {
        broadcast({ type: 'log', name, source: 'sanjang', data: `설치 중 (${config.setup})...` });
        execSync(config.setup, { cwd: wtPath, stdio: ['pipe', 'pipe', 'pipe'], timeout: 120_000, shell: true });
        broadcast({ type: 'log', name, source: 'sanjang', data: '설치 완료 ✓' });
      }
      ...
      const record = { name, branch, slot, fePort, bePort, status: 'stopped', description: description.trim() };
      upsert(record);
      broadcast({ type: 'playground-created', name, data: record });
      res.status(201).json(record);
    } catch (err) {
