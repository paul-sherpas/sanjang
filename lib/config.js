...
export function detectProject(projectRoot) {
  ...
  if (has('shadow-cljs.edn')) {
    const hasBb = has('bb.edn');
    return {
      framework: 'shadow-cljs',
      dev: { command: hasBb ? 'bb dev' : 'npx shadow-cljs watch app', port: 3000, portFlag: null, cwd: '.', env: {} },
      setup: 'npm install',
      copyFiles: findEnvFiles(projectRoot),
      _note: 'shadow-cljs detected. portFlag is null (port is configured in shadow-cljs.edn).',
    };
  }

  // Check subdirectories for shadow-cljs (monorepo)
  for (const sub of ['frontend', 'client', 'web', 'app']) {
    if (has(`${sub}/shadow-cljs.edn`)) {
      const hasBb = has(`${sub}/bb.edn`);
      return {
        framework: 'shadow-cljs',
        dev: { command: hasBb ? 'bb dev' : 'npx shadow-cljs watch app', port: 3000, portFlag: null, cwd: sub, env: {} },
        setup: 'npm install',
        copyFiles: findEnvFiles(projectRoot),
        _note: `shadow-cljs detected in ${sub}/. portFlag is null.`,
      };
    }
  }
...
function detectPackageManager(root) {
  if (existsSync(join(root, 'bun.lockb'))) return 'bun install';
