...
import { addWorktree, removeWorktree, listBranches, campPath, setProjectRoot, getProjectRoot } from './engine/worktree.ts';
import { startCamp, stopCamp, stopAllCamps, getProcessInfo, setConfig } from './engine/process.ts';
import { saveSnapshot, restoreSnapshot, listSnapshots } from './engine/snapshot.ts';
import { buildDiagnostics } from './engine/diagnostics.ts';
import { buildFallbackPrBody, buildClaudePrPrompt } from './engine/pr.ts';
import { parseConflictFiles, buildConflictPrompt } from './engine/conflict.ts';
...
function friendlyError(err: unknown, branch: string): string {
  ...
  if (/cache copy failed/.test(msg)) return '캐시 복사에 실패했습니다. POST /api/cache/rebuild 로 캐시를 다시 빌드해보세요.';
...
function setupCampDeps(name: string, wtPath: string, cfg: SanjangConfig, broadcast: (msg: BroadcastMessage) => void): void {
  ...
  broadcast({ type: 'log', name, source: 'sanjang', data: `캐시 없음 (${cacheResult.reason}), 설치 중 (${cfg.setup})...` });
...
export async function createApp(projectRoot: string, options: CreateAppOptions = {}): Promise<CreateAppResult> {
  ...
  app.post('/api/playgrounds', async (req: Request, res: Response) => {
    ...
    if (existing.length >= MAX_CAMPS) return res.status(400).json({ error: `최대 ${MAX_CAMPS}개 캠프까지 가능합니다.` });
  ...
  app.post('/api/quick-start', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const branchResult = spawnSync('git', ['branch', branch, `origin/${defaultBranch}`], { encoding: 'utf8', stdio: 'pipe' });
