...
function getHashFile(projectRoot) {
  return join(getCacheDir(projectRoot), 'lockfile.hash');
}
...
export function isCacheValid(projectRoot, setupCwd) {
  ...
  const hashFile = getHashFile(projectRoot);
...
export async function buildCache(projectRoot, config, onLog) {
  ...
  try {
    for (const modDir of allModules) {
      ...
      execFileSync('cp', ['-Rc', modDir, target], { stdio: 'pipe' });
    }
  } catch (err) {
  ...
  // Store lockfile hash
  writeFileSync(getHashFile(projectRoot), hashLockfile(lockfile.path), 'utf8');
...
export function applyCacheToWorktree(projectRoot, wtPath, setupCwd) {
  ...
  try {
    for (const cachedDir of cachedModules) {
      ...
      mkdirSync(join(target, '..'), { recursive: true });
      execFileSync('cp', ['-Rc', cachedDir, target], { stdio: 'pipe' });
    }
  } catch (err) {
